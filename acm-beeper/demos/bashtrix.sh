#! /usr/bin/env bash
# Modified from https://github.com/ara4n/random/blob/master/bashtrix.sh

# set your matrix ID & server URL, and the room you want to chat in:
USERNAME='@test:sumnerevans.com'
SERVER='https://matrix.sumnerevans.com'
ROOM=`jq -Rr @uri <<< '#testing:sumnerevans.com'` # abusing jq to uri escape
...
(while true; do
    SYNC=`curl -s "$SERVER/_matrix/client/r0/sync?access_token=$TOKEN&timeout=30000&$SINCE"`
...
while true;
    ...
    curl -s -X PUT "$SERVER/_matrix/client/r0/rooms/$ROOM_ID/send/m.room.message/m.`date +%s`?access_token=$TOKEN" --data '{"body": "'"$INPUT"'", "msgtype": "m.text"}' > /dev/null
done
